...
export const useSalesReportStore = create<SalesReportState>((set, get) => ({
  ...
  fetchSales: async (opts) => {
    ...
    try {
      ...
      const url = buildApiUrl(
        `${API_CONFIG.BASE_URL}${API_CONFIG.ENDPOINTS.SALES_REPORTS}`,
        {
          page,
          size,
      ...
      if (response.data.success) {
        ...
        set({
          error: response.data.message || "Failed to fetch sales reports",
          loading: false,
          isFetching: false,
        });
      }
      ...
      set({
        error:
          err instanceof Error ? err.message : "Failed to fetch sales reports",
        loading: false,
        isFetching: false,
      });
    }
  ...
  fetchMonthlySales: async () => {
    ...
    try {
      const token = getAuthToken();
      const url = buildApiUrl(
        `${API_CONFIG.BASE_URL}${API_CONFIG.ENDPOINTS.SALES_REPORTS}/monthly-count`
      );
      ...
      if (response.data.success) {
        ...
        set({
          error:
            response.data.message || "Failed to fetch monthly sales reports",
          loading: false,
          isFetching: false,
        });
      }
      ...
      set({
        error:
          err instanceof Error
            ? err.message
            : "Failed to fetch monthly sales reports",
        loading: false,
        isFetching: false,
      });
    }
